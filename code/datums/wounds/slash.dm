...
/datum/wound/slash/flesh/handle_process(delta_time, times_fired)
	...
	if(blood_flow < minimum_flow)
		if(demotes_to)
			...
			to_chat(victim, span_green("The cut on your [limb.name] has [!limb.can_bleed() ? "healed up" : "stopped bleeding"]!"))
			qdel(src)

/datum/wound/slash/flesh/on_stasis(delta_time, times_fired)
	if(blood_flow >= minimum_flow)
		return
	if(demotes_to)
		replace_wound(new demotes_to)
...
/datum/wound/slash/flesh/proc/lick_wounds(mob/living/carbon/human/user)
	...
	if(!do_after(user, base_treat_time, target=victim, timed_action_flags = (IGNORE_USER_LOC_CHANGE|IGNORE_TARGET_LOC_CHANGE|ENFORCE_TARGET_PROXIMITY), extra_checks = CALLBACK(src, PROC_REF(still_exists))))
		return
...
/datum/wound/slash/flesh/proc/las_cauterize(obj/item/gun/energy/laser/lasgun, mob/user)
	...
	if(!do_after(user, base_treat_time  * self_penalty_mult, target=victim, timed_action_flags = (IGNORE_USER_LOC_CHANGE|IGNORE_TARGET_LOC_CHANGE|ENFORCE_TARGET_PROXIMITY), extra_checks = CALLBACK(src, PROC_REF(still_exists))))
		return
...
/datum/wound/slash/flesh/proc/tool_cauterize(obj/item/I, mob/user)
	...
	if(HAS_TRAIT(src, TRAIT_WOUND_SCANNED))
		treatment_delay *= 0.5
		user.visible_message(span_danger("[user] begins expertly cauterizing [victim]'s [limb.name] with [I]..."), span_warning("You begin cauterizing [user == victim ? "your" : "[victim]'s"] [limb.name] with [I], keeping the holo-image indications in mind..."))
	else
...
/datum/wound/slash/flesh/proc/suture(obj/item/stack/medical/suture/I, mob/user)
	...
	if(HAS_TRAIT(src, TRAIT_WOUND_SCANNED))
		treatment_delay *= 0.5
		user.visible_message(span_notice("[user] begins expertly stitching [victim]'s [limb.name] with [I]..."), span_notice("You begin stitching [user == victim ? "your" : "[victim]'s"] [limb.name] with [I], keeping the holo-image information in mind..."))
	else
...
/datum/wound/slash/flesh/moderate
	...
	clot_rate = 0.05 //Unchanged from original TG value
	//MOJAVE EDIT END
...
/datum/wound_pregen_data/flesh_slash/laceration
	...
	threshold_minimum = 45 //MOJAVE SUN EDIT - Original TG value is 50
...
/datum/wound/slash/flesh/critical
	...
	initial_flow = 3.75 //Original TG value is 4
	minimum_flow = 3 //Original TG value is 3.85
	clot_rate = -0.01 // critical cuts actively get worse instead of better //Original TG value is -0.015
...
/datum/wound_pregen_data/avulsion/cleave
	abstract = FALSE
	can_be_randomly_generated = FALSE
